...
#
###############################################################################
#  Modified for converting vcf to fasta file, Qinhu Wang
###############################################################################
...
my $vcf_file = $global_options->{'vcf'};
...
while(my $sobj = $seqio->next_seq)
{
    ...
    $seq=vcf_snp($vcf_file, $seq, $seqid);
...
sub vcf_snp {
	my ($vcf_file, $seq, $seqid) = @_;

	my %vcf = ();
	open (IN, $vcf_file) or die "Cannot open file $vcf_file: $!\n";
	while (<IN>) {
		chomp;
		next if /^#/;
		my @w = split /\t/;
...
sub checkParams {
    ...
    if(!exists $options{'vcf'} ) { printParamError ("You MUST supply a VCF file to parse"); }
...
---------------------------------------------------------------- 
 $0
 Copyright (C) Michael Imelfort, Kranti Konganti, and Qinhu Wang
...
=head1 NAME
	...
	vcf2fasta.pl (based on gff2fasta.pl)
...
=item * 11/21/2015
   ...
   gff2fasta now adapted for vcf2fasta
...
=head1 DESCRIPTION
   ...
   Convert a VCF file to fasta

=head1 SYNOPSIS

    vcf2fasta.pl -gff|g GFF -vcf|v VCF -fasta|f FASTA -out|o FASTA

      -gff -g GFF                  GFF3 file to parse
      -vcf -v VCF                  VCF file to parse
