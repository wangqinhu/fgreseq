library(squash)
pdf("data/go/go_up10.pdf",width = 5.6,height = 3.2)

layout(matrix(c(1,1,1,4,2,3),2,3,byrow = TRUE),c(1.5,1,1),c(5,1))

go<-read.table("data/go/go_up10.reduced.txt", header = T, sep = "\t", check.names = T)
...
rect(max(gof)+1.2,2-0.5,137,13-0.5,col="#FFFFCC", border = "white")
rect(max(gof)+1.2,13-0.5,137,19-0.5,col="#FFCCCC", border = "white")

points(gof,seq(1,length(go$GOID)),
     col=jet(18)[rank(go$T)],
     pch=16,
...
abline(v=132,col="white", lwd=2)
abline(v=max(gof)+1)
text(134.5,1,"CC",cex=0.6)
text(134.5,7,"MF",cex=0.6)
text(134.5,15.5,"BP",cex=0.6)
box()
...
text(0,-2.7,"-log10(p-value)", adj=0.75)
