...
x<-as.matrix(cbind(sps$density,spf$density))
colnames(x)<-c("slow","fast")
row.names(x)<-c("0","1","2","3","4")
library(RColorBrewer)
mc<-brewer.pal(5,"Blues")
mp<-barplot(x, col=mc, ylab="Ratio",xlim=c(0, 4.5))
legend("right",rev(rownames(x)),pch=16, cex=1,col=rev(mc),title = "SP/25Kb")
